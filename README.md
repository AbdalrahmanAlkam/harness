...
The opt-in swarm coordinator runs Architect and QA planning concurrently, gives the Coder sole write access to the isolated worktree, then runs QA verification and Security review concurrently. The QA result must include a successful test-tool observation. Set `/swarm on|off|auto`; `auto` uses it only for deep editing tasks. The telemetry sidebar shows worktree and role status. Swarm results are structured artifacts passed between phases, so no role shares a mutable conversation with another.
