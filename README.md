...
For complex editing tasks in a Git workspace, the TUI creates `.harness/worktrees/task-ID` from the current `HEAD` and roots file, shell, and test tools there. The main checkout is not edited until review. Use `/isolation on|off|auto` to control this; `auto` selects multi-step editing tasks. After a verified result, F3 or `/diff` shows the complete patch. Enter or **Merge & Apply** applies it to a clean, unchanged main checkout; Escape or **Discard & Abort** removes the temporary worktree and branch. If the main checkout has changed or is dirty, apply is refused and the worktree remains available for review. Exiting with a pending review discards it. Worktree startup time depends on repository size and disk performance.
...
The opt-in swarm coordinator runs Architect and QA planning concurrently, gives the Coder sole write access to the selected worktree or direct workspace, then runs QA verification and Security review concurrently. The QA result must include a successful test-tool observation. Set `/swarm on|off|auto`; `auto` uses it only for deep editing tasks. The telemetry sidebar shows worktree and role status. Swarm results are structured artifacts passed between phases, so no role shares a mutable conversation with another. A subagent exception is retried once before its phase fails.

Requests that explicitly mention multiple agents also enter swarm mode automatically. Headless `adaptive-harness dev --swarm "TASK"` exposes `delegate_subagent` for architect, coder, and reviewer roles. The Coder can write files and run build commands inside the selected workspace. When a model returns fenced code with an explicit `path:` header, the agent writes it using `write_file`; a plain code answer never counts as a completed edit.
