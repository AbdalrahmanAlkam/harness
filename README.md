...
Swarm mode defaults to `auto`. Explicit multi-agent requests and large multi-part editing tasks run Architect, Coder, and Reviewer in sequence. The Architect plans with read tools; the Coder has all core coding tools and sole write access to the launch workspace or selected worktree; the Reviewer inspects files and runs checks. A planning warning does not block the Coder. The sidebar shows all three role states, and a failed Coder run is retried once.
